<?php 
 $cocuk_form = "hidden";
		...
		$cins = $_POST["cins"];
		$email = $_POST["email"];
		$sql = "UPDATE uye SET ad='$ad', soyad='$soyad', sehir='$sehir', dyil=$dyil, cins='$cins', email='$email' WHERE uid=$uid";
	...
	/* post yoluyla gleen bilgiyle kayıt silme */
...
?>
               	...
               	<form id="yeni" name="yeni" method="post" action="yonetim.php">
...
<OPTION <?php if($dizi[3]=="99") echo "selected"; ?> VALUE="99">YABANCI</OPTION>
        	    ...
        	    <label>Eposta Adresiniz
        	      <input type="text" name="email" id="email"  value="<?php echo($dizi[6]); ?>"/>
      	      </label><br /></td>
